...
export default function AdminProductList() {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    console.log('handleInputChange called:', { name, value, elementType: e.target.tagName });
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.startsWith('image/')) {
      setErrorMessage('Please select a valid image file (PNG, JPG, JPEG, GIF, WebP).');
      return;
    }
  ...
  const handleVariablesCRUD = async (productId: string, currentVariables: any[], newVariables: any[]) => {
    try {
      // Delete removed variables
      const currentIds = new Set(currentVariables.map(v => v.id).filter(Boolean));
  ...
  const handleSubmit = async () => {
    ...
    let cleanedImage = '';
    if (form.image && (form.image.startsWith('data:image') || form.image.startsWith('http'))) {
    ...
    try {
      if (editingId) {
        ...
        await handleVariablesCRUD(editingId, originalVariablesRef.current, form.productVariables);
        
        setMessage('Product updated successfully.');
      } else {
        // Create new product
        const newProduct = await createProduct({ ...productPayload, productVariables: [] });
  ...
  const handleEdit = (product: any) => {
    ...
    setImagePreview(imageUrl && (imageUrl.startsWith('http') || imageUrl.startsWith('data:')) ? imageUrl : '');
...
<div className="overflow-x-auto border rounded-lg">
  <table className="min-w-full text-sm text-left">
    <thead className="bg-gray-100 text-gray-700">
      <tr>
        ...
        <th className="px-4 py-3 font-medium">Total Quantity</th> {/* New Column */}
        <th className="px-4 py-3 font-medium">Category Name</th> {/* New Column */}
        ...
        paginatedProducts.map(product => (
          <tr key={product.id} className="border-b hover:bg-gray-50 transition-colors">
            <td className="px-4 py-3">
              {truncateProductName(product.product_name)} {/* Truncated Name */}
            </td>
            <td className="px-4 py-3">
              <ProductImage
            ...
            <td className="px-4 py-3">${product.original_price?.toFixed(2) || '0.00'}</td>
            <td className="px-4 py-3">{product.discount_percentage_tag || 0}%</td>
            <td className="px-4 py-3">${(product.discounted_price || product.original_price || 0).toFixed(2)}</td>
            <td className="px-4 py-3">
              {truncateDescription(product.description)} {/* Truncated Description */}
            </td>
            <td className="px-4 py-3">{product.total_quantity || 0}</td> {/* Display total_quantity */}
            <td className="px-4 py-3">{product.category?.category_name || 'No category'}</td> {/* Display category_name */}
            <td className="px-4 py-3 text-xs text-gray-500">
              {product.created_at ? dayjs(product.created_at).format('MMM DD, YYYY') : 'N/A'}
            </td>
            <td className="px-4 py-3 text-xs text-gray-500">
              {product.updated_at ? dayjs(product.updated_at).format('MMM DD, YYYY') : 'N/A'}
            </td>
                    ...
                    >
                      {categories
                        ...
                        .map(cat => (
                          <div
                            key={cat.id}
                            onClick={() => handleCategorySelect(cat.id, cat.category_name)}
  ...
  {form.productVariables.length === 0 && (
    <p className="text-sm text-gray-500 mb-2">No product variables added.</p>
  )}
    ...
    <div key={index} className="grid grid-cols-12 gap-4 mb-2 items-center">
      <div className="col-span-5">
        <input
          ...
          placeholder="Size"
          value={variable.size}
          onChange={(e) => handleVariableChange(index, 'size', e.target.value)}
          className="appearance-none block w-full px-4 py-3 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 transition"
        />
      </div>
      <div className="col-span-5">
        <input
          ...
          onChange={(e) =>
            handleVariableChange(index, 'quantity', e.target.value)
          }
          className="appearance-none block w-full px-4 py-3 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 transition"
        />
      </div>
          ...
          <div className="flex items-start">
            ...
            <p className="text-sm font-medium text-red-800">{errorMessage}</p>
                ...
                <div className="flex justify-end gap-3 pt-6 border-t border-gray-200">
                  ...
                  >
                    {isUploading ? 'Processing...' : (editingId ? 'Update Product' : 'Create Product')}
                  </button>
                </div>
              </div>
            </div>
