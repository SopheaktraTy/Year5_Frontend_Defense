...
import { ProductDto, ProductVariableInProductDto } from '../../types/productType';
import { Minus, Plus, RulerDimensionLine, Shield, Mail,Ship, CircleFadingArrowUp } from 'lucide-react';

interface ProductDetailPageProps {
  onProductLoaded?: (productName: string) => void;
}

const ProductDetailPage: React.FC<ProductDetailPageProps> = ({ onProductLoaded }) => {
    ...
    if (productId && typeof productId === 'string') {
      getProductById(productId)
        .then((res) => {
          ...
          setProduct({
            ...data,
            product_variables: Array.isArray(data.product_variables) ? data.product_variables : [],
          });
          if (onProductLoaded) {
  ...
  const discountedPrice = typeof product.discounted_price === 'number' ? product.discounted_price : 0;
  ...
  const currentStock = selectedSize ? selectedVariable?.quantity ?? 0 : product.total_quantity;
  ...
  return (
    ...
    <div className="max-w-7xl mx-auto px-4 py-6 grid grid-cols-1 md:grid-cols-2 gap-10 bg-white rounded-lg">
      ...
      <div className=''>
        <h1 className="text-3xl md:text-4xl font-semibold mb-4">{product.product_name}</h1>
...
{originalPrice !== discountedPrice && (
  <p className="text-red-500 mb-4 text-sm">
    Discount: ${discountAmount} ({product.discount_percentage_tag}%)
        ...
        <p className="font-normal text-base text-gray-600 mb-4">{product.description}</p>
        ...
        <div className="mb-4">
          <p className="font-medium text-gray-700 mb-2 text-base">Size:</p>
  ...
  <button
    className="flex-1 bg-gradient-to-r bg-blue-600 text-white rounded-full py-3 px-6 text-sm font-semibold disabled:opacity-50"
    disabled={
      product.product_variables.length > 0
...
<hr className="my-4 border-gray-200 rounded-lg" />
        <div className="font-normal text-base text-gray-600 mb-6 flex flex-col">
          ...
          <Ship size={16}/>
          <p>
            Estimated Delivery: <strong className='font-bold text-base text-gray-700'>Jul 26 - Jul 30</strong>
          </p>
          ...
          <p>
            Return within <strong className='font-bold text-base text-gray-700'>30 days</strong> of purchase. Taxes are non-refundable.
          </p>
          </div>
        </div>
