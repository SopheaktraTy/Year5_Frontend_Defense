...
const CustomerProductSectionPage: React.FC = () => {
  const [sectionPages, setSectionPages] = useState<ProductSectionPageDto[]>([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)

  // ✅ Same scrollRefs structure as admin version
  const scrollRefs = useRef<Record<string, HTMLDivElement | null>>({})

  // ✅ Same scroll state structure as admin version
  ...
  /** 🔄 Update scroll state for a section */
  ...
  /** ⬅️➡️ Scroll by 320px (like admin) */
      ...
      {sectionPages.map(section => (
        <div key={section.id} className="rounded-lg border-gray-200 w-full">
          ...
          <div className="flex justify-between items-center mb-3 bg-white p-4 rounded-lg w-full">
            ...
            <Link
              href={`/customer/section/${section.id}`}
...
export default CustomerProductSectionPage
